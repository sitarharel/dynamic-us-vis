function State(svg, map, data, width, height) {
    ...
    var locationScale = function(d) {
        return locationCoordinates[d]; }
    ...
    var cleanData = data.reduce((a, x) => { a[+x.STATE] = x;
        return a }, []);
    ...
    var state_mapping = {
        ...
        "layout": {
            ...
            "shape_duration": 500,
            "location": (d, i) => {
                return locationScale(this.get_data(d.id)) },
            "tween": [
        ...
        "graph": {
            ...
            "shape_duration": 500,
            "location": (d) => {
                return [graphXScale(this.get_data(d.id)), graphYScale(this.get_compared_to_data(d.id))]; },
            "tween": [
                { style: "opacity", f: (d) => 0.8 },
    ...
    this.set_map_state = function(state_name, options) {
        ...
        if (this.current_state == "graph") {
            this.current_state = d3.select('input[name="point-shape"]:checked').node().value;
        }
        ...
        var stateOptions = state_mapping[this.current_state] || state_mapping["default"];
        ...
        if (this.current_state == "graph" || this.current_state == "graph_circle") {
            this.draw_axises();
            d3.select("#point_shape").style("display", "block");
            d3.selectAll(".graph-dataset").style("display", "inline-block");
        } else {
            this.remove_axises();
            d3.select("#point_shape").style("display", "none");
            d3.selectAll(".graph-dataset").style("display", "none");
        }

        if (this.current_state == "default" || this.current_state == "layout") {
            d3.select("#sort_order").style("display", "block");
        } else {
            d3.select("#sort_order").style("display", "none");
        }
    }
    ...
    this.set_domains = function() {
        ...
        if (this.current_state == "default" || this.current_state == "layout") {
            if (d3.select("#sort_order select").node().value == "ASCENDING") opacityRange = [0.9, 0.1]
        }

        opacityScale = d3.scaleLinear().range(opacityRange).domain(extent);
    ...
    this.get_data = function(state_fips) {
        ...
        keys.sort(function(a, b) {
            return columnData[b] - columnData[a] });
        if (d3.select("#sort_order select").node().value == "ASCENDING") {
            keys.reverse();
        }
    ...
    this.set_data = function(column, options) {
        ...
        d3.select("#color_" + color.slice(1)).classed("active", true);
    ...
    this.create_scale = function() {
        ...
        legend.append("text")
            ...
            .attr("x", 80)
            .attr("y", function(d, i) {
                return height - (i * ls_h) - ls_h - 4 - 350; })
            .text(function(d, i) {
                return d3.format(",.2f")(opacityScale.invert(d)); });
    }
